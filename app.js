...
function applyLang() {
  ...
  const setQ = (sel, val) => { const el = qs(sel); if (el) el.textContent = val; };

  set('logoSub', t('logoSub'));

  const ug = document.getElementById('userGreeting');
  if (ug) ug.innerHTML = name ? `${t('greeting')}, <span>${name}</span>` : '';

  setQ('.streak-label', t('streakLabel'));
  setQ('.panel-title',  t('panelTitle'));
  setQ('.add-habit-btn', t('addBtn'));
  setQ('.all-done-title', t('allDoneTitle'));
  setQ('.all-done-sub',   t('allDoneSub'));

  const statLabels = qsa('.stat-label');
  ['statDays','statPerfect','statHabits'].forEach((k, i) => {
  ...
  t('calWeekdays').forEach((d, i) => { if (wds[i]) wds[i].textContent = d; });

  setQ('.modal-close', t('closeBtn'));
  ...
  ['nameLabel','descLabel','timeLabel','iconLabel','colorLabel'].forEach((k, i) => {
    if (formLabels[i]) formLabels[i].textContent = t(k);
  });

  setQ('.btn-secondary', t('cancelBtn'));
  setQ('.btn-primary',   t('saveBtn'));

  set('ob-tagline', t('obTagline'));
  set('ob-label',   t('obLabel'));
  set('ob-btn',     t('obBtn'));
...
// ════════════════════════════════════════════════
// DATE DISPLAY — fully translated
// ════════════════════════════════════════════════
function updateDateDisplay() {
  const n      = new Date();
  const tr     = TRANSLATIONS[currentLang] || TRANSLATIONS.en;
  const day    = tr.calDayNames[n.getDay()];
  const date   = n.getDate();
  const month  = tr.calMonths[n.getMonth()];
  const year   = n.getFullYear();
  const el     = document.getElementById('dateDisplay');
  if (el) el.textContent = tr.dateFmt(day, date, month, year);
}

// ════════════════════════════════════════════════
// BACKGROUND — rotates daily
// ════════════════════════════════════════════════
const BG_IMAGES = [
  'images/DO MORE.jpg',
  'images/HD Wallpaper For PC.jpg',
...
function setDailyBackground() {
  ...
  const img = BG_IMAGES[dayOfYear % BG_IMAGES.length];
...
function updateMotivation() {
  ...
  if (el) el.innerHTML = pool[day % pool.length];
}
...
function renderHabits() {
  ...
  if (HABITS.length === 0) {
    list.innerHTML = `
      <div style="text-align:center;padding:40px 20px;opacity:0.5;">
        <div style="font-family:'Cormorant Garamond',serif;font-size:20px;font-style:italic;color:#fff;margin-bottom:8px;">${t('emptyTitle')}</div>
        <div style="font-family:'DM Mono',monospace;font-size:10px;letter-spacing:1.5px;color:#b0b8c8;">${t('emptySub')}</div>
      </div>`;
    updateProgress();
    updateStats();
    return;
  ...
  HABITS.forEach(habit => {
    ...
    card.innerHTML = `
      ...
      <button class="habit-delete" title="Delete" onclick="deleteHabit(event,'${habit.id}')">×</button>
    `;
...
function toggleHabit(habitId) {
  const state = getTodayState();
  if (state.habits[habitId]) delete state.habits[habitId];
  else state.habits[habitId] = Date.now();
...
function updateStats() {
  ...
  while (true) {
    ...
    const day = appData[k];
    if (day && Object.keys(day.habits || {}).length > 0) { streak++; cur.setDate(cur.getDate() - 1); }
    else break;
  }
...
function renderCalendar() {
  const tr = TRANSLATIONS[currentLang] || TRANSLATIONS.en;
  document.getElementById('calMonthLabel').textContent = `${tr.calMonths[calViewMonth]} ${calViewYear}`;

  const wds = document.querySelectorAll('.cal-wd');
  tr.calWeekdays.forEach((d, i) => { if (wds[i]) wds[i].textContent = d; });
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    el.addEventListener('click', () => { selectedDay = dateStr; renderCalendar(); openDayModal(dateStr); });
    daysEl.appendChild(el);
  }
...
function openDayModal(dateStr) {
  ...
  const tr         = TRANSLATIONS[currentLang] || TRANSLATIONS.en;
  const dayName    = tr.calDayNames[dateObj.getDay()];
  const monthName  = tr.calMonths[dateObj.getMonth()];
  document.getElementById('modalDate').textContent = tr.dateFmt(dayName, d, monthName, y);
